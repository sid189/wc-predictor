...
export default async function FriendliesPage() {
  ...
  const [{ data: matches }, { data: teams }, { data: myPreds }, { data: results }] =
    await Promise.all([
      supabase.from("matches").select("*").eq("stage", "friendly").order("kickoff_at"),
  ...
  if (!matches || matches.length === 0) {
    return (
      <div className="space-y-3">
        <h1 className="text-xl font-semibold">Friendlies</h1>
        <p className="text-sm text-zinc-500">
          No friendly matches scheduled yet. An admin can add them.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500">
          Warmup matches to test the prediction flow before WC kickoff. Same scoring rules; ET &amp; penalty predictions allowed.
        </p>
      </div>
              ...
              >
                <div className="flex-1">
                  <div className="text-xs text-zinc-500">
                    Friendly · <LocalTime iso={m.kickoff_at} preset="datetime" />
                  </div>
                  <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 font-medium">
                    <span className="inline-flex items-center gap-1.5">
                      <Flag teamName={name(m.team_a_id)} size={18} />
                      {name(m.team_a_id)}
                    </span>
                    <span className="text-zinc-400">vs</span>
                    <span className="inline-flex items-center gap-1.5">
                      <Flag teamName={name(m.team_b_id)} size={18} />
                      {name(m.team_b_id)}
                    </span>
                  </div>
                </div>
                <div className="text-right text-sm">
                  ...
                  <div className="text-xs">
                    {win.state === "locked" ? (
                      <span className="text-zinc-400">{hasKickedOff(m.kickoff_at) ? "locked" : ""}</span>
                    ) : win.state === "open" ? (
